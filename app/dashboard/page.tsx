  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-24">
      <div className="sticky top-0 bg-white/85 backdrop-blur-xl border-b border-slate-200/70 z-10 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="font-bold text-slate-800 tracking-tight">収支ダッシュボード</h1>
      </div>

      <div className="max-w-lg mx-auto px-4 pt-4 space-y-4">
        {/* サマリーカード */}
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm p-3 text-center">
        ...
        {months.length === 0 ? (
          <div className="text-center py-20 text-slate-400">
            <span className="inline-flex w-16 h-16 bg-slate-100 rounded-3xl items-center justify-center mb-3">
              <PieChart size={28} className="text-slate-300" />
            </span>
            <p className="text-sm font-medium">まだデータがありません</p>
            <p className="text-xs mt-1">スキャン後に「履歴に保存」してください</p>
          </div>
        ) : (
          months.map((m) => (
            <div key={m.month} className="bg-white rounded-3xl border border-slate-200/60 shadow-sm p-4">
